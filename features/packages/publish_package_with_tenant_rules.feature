@articles
@disable-fixtures
Feature: Publish article based on configured organization and tenant rules
  In order to publish article based on configured organization and tenant rules
  As a HTTP Client
  I want to be able to send ninjs payload and publish article so package status will be set to published
  ...
  Scenario: Publish article
    ...
    Then the response status code should be 200
    And the JSON node "_embedded._items[0].status" should be equal to "published"
