...
Feature: Listing evaluated rules which match the package's metadata
  ...
  Scenario: Evaluating rules when there are organization rules only defined
    ...
    And I add "Content-Type" header equal to "application/json"
    Then I send a "POST" request to "/api/{version}/rules/evaluate" with body:
     """
    ...
    And the JSON node "[0].organization.code" should be equal to "123456"
    And the JSON node "[0].tenants[0].tenant.code" should be equal to "123abc"
    And the JSON node "[0].tenants[0].routes" should not exist
  ...
  Scenario: Listing rules when there are organization and tenant rules defined
    ...
    And the JSON node "[0].organization.code" should be equal to "123456"
    And the JSON node "[0].tenants[0].tenant.code" should be equal to "123abc"
    And the JSON node "[0].tenants[0].routes" should exist
    And the JSON node "[0].tenants[0].routes[0].id" should be equal to "3"
