...
Feature: Registering and login as new user publisher
  ...
  Scenario: Registering new user
    ...
    When I send a "PATCH" request to "/api/v2/tenants/123abc" with body:
     """
      {
          "pwa_config": {
            "url": "http://pwa_url.local:80"
          }
      }
    """
    Then the response status code should be 200
    And the JSON node "pwa_config.url" should be equal to "http://pwa_url.local:80"
