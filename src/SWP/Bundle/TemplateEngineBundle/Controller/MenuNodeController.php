...
class MenuNodeController extends FOSRestController
{
     ...
     *     }
     * )
     * @Route("/api/{version}/templates/menunodes/{menuName}", options={"expose"=true}, defaults={"version"="v1"}, name="swp_api_templates_list_menu_nodes")
     * @Method("GET")
     */
    public function listAction(Request $request, $menuName)
    {
        if (!$menuName) {
        ...
        $qb->from('Symfony\Cmf\Bundle\MenuBundle\Doctrine\Phpcr\MenuNode', 'm');

        $path = $this->getBaseDocumentPath().'/'.$menuName;
        $qb->where()->descendant($path, 'nodes');
        $query = $qb->getQuery();
     ...
     *     }
     * )
     * @Route("/api/{version}/templates/menunodes/{menuName}/{id}", options={"expose"=true}, defaults={"version"="v1"}, name="swp_api_templates_get_menu_node")
     * @Method("GET")
     */
